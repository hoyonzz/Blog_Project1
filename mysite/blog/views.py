...
def category_page(request, slug):
    ...
    context = {
        'blog_list': Post.objects.filter(category=category).order_by('-pk'),
        'categories': categories,
        'no_category_post_count': Post.objectsfilter(category=None).count(),
        'category': category,
...
class PostCreate(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if current_user.is_authenticated:
            ...
            # 기존에 리턴하던 값을 response에 담아둠
            response = super(write, self).form_valid(form)
...
def new_comment(request, pk):
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                # commit=Falser를 하면, db에 바로 저장되지 않고, 메모리에 객체 하나 생성해줌.
...
def delete_comment(request, pk):
    if request.user.is_authenticated:
        ...
        if comment.author == request.user:
            ...
            return HttpResponse('You ar not allowed to delete this comment')
    return redirect('/blog/')
